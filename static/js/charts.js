...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var result = data.samples;

    // 4. Create a variable that filters the samples for the object with the desired sample number.
    var resultArray = result.filter(sampleObj => sampleObj.id == sample);

    // 1. Create a variable that filters the metadata array for the object with the desired sample number.
    var metadata = data.metadata.filter(metadataObj => metadataObj.id == sample);

    //  5. Create a variable that holds the first sample in the array.
    var samples = resultArray[0];

    // 2. Create a variable that holds the first sample in the metadata array.
    metadata = metadata[0];
    
    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    // Get the top 10 otu_ids and map them in descending order  
